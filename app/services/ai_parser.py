...
- Факты: МАКСИМАЛЬНО АГРЕССИВНО вытаскивай любые важные факты из ответов (цели по похудению, аллергии, травмы, расписание, хобби, имена близких) и сохраняй в `memories`. Если пользователь отвечает на стартовое интервью, ОБЯЗАТЕЛЬНО запиши все его ответы как факты.
